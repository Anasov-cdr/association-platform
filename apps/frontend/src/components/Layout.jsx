      ...
      <header className="sticky top-0 z-20 border-b border-[#d9eef5] bg-gradient-to-r from-white/96 via-[#eefbfc]/96 to-[#d8f5f7]/96 text-ink shadow-lg shadow-moss/10 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-3 px-4 py-2">
                ...
                <div className="absolute left-0 top-full pt-2">
                  <div className="w-56 rounded-md border-2 border-[#c9e9f2] bg-[#f8fdff] p-2 text-ink shadow-[0_18px_44px_rgba(15,126,168,0.2)]">
                    {servicesNav.map(([key, href]) => (
                      <NavLink key={key} to={href} className={dropdownLinkClass}>{t(`nav.${key}`)}</NavLink>
                    ))}
                  </div>
                </div>
              )}
          ...
          <div className="flex items-center gap-3">
            <div className="hidden rounded bg-[#e8fbfc] px-3 py-1 text-xs font-black uppercase text-moss md:block">{roleLabels[role] || role}</div>
          ...
          <div className="border-t border-[#d9eef5] bg-gradient-to-r from-white/98 via-[#eefbfc]/98 to-[#d8f5f7]/98 px-4 pb-4 lg:hidden">
            <div className="mx-auto max-w-7xl">
              <div className="mb-3 flex items-center justify-between gap-3 pt-4">
                <div className="rounded bg-[#e8fbfc] px-3 py-1 text-xs font-black uppercase text-moss">{roleLabels[role] || role}</div>
        ...
        <div className="mx-auto grid max-w-7xl gap-8 px-4 py-10 md:grid-cols-[1.35fr_0.9fr_0.9fr_1.05fr]">
          <section>
            <Link to={`/${lang}`} className="inline-flex items-center gap-4">
              ...
              <span className="font-display text-xl font-bold leading-tight">Ассоциация выпускников</span>
            </Link>
            <p className="mt-4 max-w-md text-sm font-medium leading-7 text-white/78">
              Платформа объединяет выпускников, студентов, работодателей и администрацию колледжа: каталог, карьера, менторство, мероприятия, документы и поддержка инициатив.
            </p>
            <div className="mt-5 flex flex-wrap gap-2">
              <Link to={`/${lang}/register`} className="rounded bg-white px-4 py-2 text-sm font-bold text-moss">Стать участником</Link>
              <Link to={`/${lang}/donations`} className="rounded border border-white/45 px-4 py-2 text-sm font-bold text-white hover:bg-white/12">Поддержать</Link>
            </div>
          </section>

          <section>
            <h2 className="text-sm font-black uppercase tracking-[0.22em] text-white/58">Платформа</h2>
          ...
          <section>
            <h2 className="text-sm font-black uppercase tracking-[0.22em] text-white/58">Сервисы</h2>
            <nav className="mt-4 space-y-3">
          ...
          <section>
            <h2 className="text-sm font-black uppercase tracking-[0.22em] text-white/58">Контакты</h2>
            <div className="mt-4 space-y-3 text-sm font-medium leading-6 text-white/80">
              ...
              {site.website && <a href={site.website} target="_blank" rel="noreferrer" className={footerLinkClass}>Официальный сайт колледжа</a>}
              {site.workingHours && <p>{site.workingHours}</p>}
            </div>
          </section>
        </div>
        <div className="border-t border-white/16">
          <div className="mx-auto flex max-w-7xl flex-col gap-3 px-4 py-5 text-sm font-semibold text-white/62 md:flex-row md:items-center md:justify-between">
            <span>© {new Date().getFullYear()} Ассоциация выпускников БФЭТ им. А. Токтоналиева</span>
            <span>Контент управляется через админ-панель платформы</span>
          </div>
        </div>
      </footer>
    </div>
